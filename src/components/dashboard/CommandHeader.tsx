    ...
    <div className="flex items-center gap-2.5" role="status" aria-label={`Data feed ${status}`}>
      <div className="relative flex items-center justify-center w-4 h-4">
        ...
        {status === 'live' && (
          <div
            ref={ringRef}
            className={`absolute w-1.5 h-1.5 rounded-full ${colorMap[status]}`}
          />
        )}
      </div>
...
function DataCell({
  ...
  return (
    ...
    >
      ...
      <div className="absolute top-0 left-0 right-0 h-[1px] bg-gradient-to-r from-[#d4f000]/80 to-[#d4f000]/20 scale-x-0 group-hover:scale-x-100 transition-transform duration-300 origin-left" />
  ...
  return (
    <header ref={headerRef} className="relative w-full bg-[#0a0a0a]/80 backdrop-blur-xl border-b border-[#333]/40 overflow-hidden">
      ...
      <div className="absolute inset-0 pointer-events-none" style={{
        background: 'radial-gradient(ellipse 40% 60% at 20% 50%, rgba(212, 240, 0, 0.015), transparent 70%), radial-gradient(ellipse 30% 50% at 80% 50%, rgba(74, 144, 226, 0.012), transparent 70%)',
      }} />
      {/* Top gradient accent line */}
      <div className="absolute top-0 left-0 right-0 h-[2px] bg-gradient-to-r from-[#d4f000]/5 via-[#d4f000]/25 to-[#d4f000]/5 z-10" />
      ...
      <div className="flex justify-between items-center px-5 lg:px-8 py-3 border-b border-[#333]/20">
        <div className="flex items-center gap-4">
          ...
          <div className="hidden md:flex md:items-center md:gap-2 font-mono text-[8px] uppercase tracking-[0.2em] text-[#333]/80">
            <span>Intelligence Dashboard</span>
            <span className="text-[#d4f000]/20 font-black px-1.5 py-0.5 bg-[#d4f000]/[0.03] border border-[#d4f000]/[0.06] text-[7px] tracking-[0.1em]">v2.0</span>
          </div>
        </div>
        <div className="flex items-center gap-4">
          <LiveClock />
      ...
      {/* Data grid */}
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 divide-x divide-[#333]/15 wr-header-grid-glow" role="region" aria-label="Live price data" aria-live="polite">
